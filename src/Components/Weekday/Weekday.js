...
const Weekday = ({ OnClickHandler, date, activeDay }) => {
  ...
  return (
    <DateLink
      style={activeStyle({
        borderBottom: `3px solid ${({ theme: { colors } }) => colors.graphit};`,
      })}
    ...
    >
      <WeekdayContainer
        style={
          dateformat(day) === dateformat(today)
            ? {
                padding:"5px 10px"
              }
            : { "": "" }
      ...
      >
        <span style={activeStyle({ color: `white` })}>
          {day.toLocaleDateString("en", { weekday: "short" }).toUpperCase()}
        </span>
        <p style={activeStyle({ color: `white`, fontSize: "26.5px" })}>
          {dateformat(day) !== dateformat(today)
            ? day.getDate() > 9
              ? day.getDate()
              : "0" + day.getDate()
        ...
        <span style={activeStyle({ color: `white` })}>
          {dateformat(day) !== dateformat(today)
            ? day.toLocaleDateString("en", { month: "short" }).toUpperCase()
