...
import { ClubLogo,ClubLink, Date,NameAndLogo,MatchCard,Centered, Line, LiveContainer, Scores,InfoContainer, StarFillLogo, StarOutLogo, MatchLink} from './Match.styled';
    ...
    return (
       <MatchLink to={"/match/"+ id}> <MatchCard >
            ...
            <NameAndLogo style={{
                    ...
                    letterSpacing:parseInt(homeScore) > parseInt(awayScore)?'1px':''
            }}>
                <ClubLink onClick={()=>getStandings(leagueId) } to={"/team/"+ homeId}
                >{homeName} </ClubLink>
                <ClubLogo onError={()=>SetSrcHome(DefaultClubLogo)}  src={SrcHome}/>
            ...
            <ClubLogo onError={()=>SetSrcAway(DefaultClubLogo)}  src={SrcAway}/> 
            <ClubLink onClick={()=>getStandings(leagueId)} to={"/team/"+ awayId}>
            {awayName} </ClubLink>
            </NameAndLogo> 
