...
const Lineups = () => {
  const { activeMatch, player } = useMatches();
  ...
  let {
    starting_lineups: startHomeLineup,
    substitutes: homeSubstitutes,
    coach: homeCoach,
  } = homeLineup;
  let {
    starting_lineups: startAwayLineup,
  ...
  return (
    <>
      <Link
        ...
        to={"/player/" + teamPlayer.player_key}
        onClick={() => getPlayer(teamPlayer.player_key)}
      >
        <Player where={where}>{teamPlayer.lineup_player}</Player>{" "}
      </Link>
    </>
  );
};
