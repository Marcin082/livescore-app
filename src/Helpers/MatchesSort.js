...
const MyLeagues = [
  1, 19, 28, 25, 27, 24, 372, 3, 4, 683, 633, 152, 302, 207, 175, 168, 259,
];
export const SortByLeague = (Matches) => {
  ...
  for (let i = 0; i < Matches.length; i++) {
    if (!ids.includes(Matches[i].league_id)) {
      LeaguesMatches.push(
        Matches.filter((league) => Matches[i].league_id === league.league_id)
      );
...
export const AddTopLeagues = (Matches) => {
  ...
  for (let i = 0; i < Matches.length; i++) {
    if (!ids.includes(Matches[i].country_id)) {
      LeaguesMatches.push(
        Matches.filter((league) => Matches[i].country_id === league.country_id)
      );
